...
def get_last_successful_fetch(
    client: Client, fetch_type: str | list[str], provider_name: str | None = None
    ...
    `provider_name`, when given, narrows further -- e.g. `"fo_edge_nse"`
    vs `"fo_edge_bse"` (see supabase/functions/fo-refresh/index.ts) so the
    Dashboard can show "Last NSE F&O refresh"/"Last BSE F&O refresh"
    separately rather than one combined F&O timestamp."""
