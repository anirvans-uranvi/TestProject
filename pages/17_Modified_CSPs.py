"""Modified CSPs -- every stock-bucket Trade **with at least one option
leg and no stock holding at all** whose Trade Type is **neither "CSP"
nor "Portfolio "-prefixed** (`portfolio_service.is_other_trade_type`): a
bare Strangle/Jade Lizard/Twisted Sister/IC, or the default "Trade"
label on a naked option position -- the shapes a CSP typically turns
into once it's been rolled/adjusted rather than assigned. Nested in the
"Wheel Strategy" journey right after My Current CSPs: Screener for CSP
-> My Current CSPs -> **this page** -> My Portfolio Trades (holdings
*with* option legs) -> Other Stock Holdings (holdings with none).

**Any Trade with a stock holding is excluded entirely**, per an
explicit follow-up request -- `not any(leg["leg_type"] == "Holding" for
leg in t["legs"])`. This is a hard requirement, not just "the Stock
Holding block stays blank": a hand-typed, non-Portfolio-prefixed label
on a Trade that genuinely pairs a holding with option legs (e.g.
`"Hedged"`) used to show up here with its Stock Holding numbers
populated; now it doesn't show up at all (rename it to start with
`"Portfolio "` on Analyse Trade to have it show up on My Portfolio
Trades instead). Combined with requiring at least one option leg, every
Trade here now has *only* Position legs -- no Holding leg whatsoever --
which is also why the Stock Holding-derived columns (Total P&L, Stock
Avg Price, Stock Qty, Stock Invested) were dropped entirely: they'd
either be identical to the option-only figures already shown (Total
P&L) or permanently blank (the rest).

This page, under an earlier name and a wider scope, went through two
renames and one reordering, all per separate explicit user requests:
1. **"Other Stock Trades"**, nested *after* Other Stock Holdings --
   modeled on the deleted `pages/13_My_Other_Trades.py` ("Other
   Trades"), reusing `portfolio_service.is_other_trade_type` (restored
   unchanged) but narrowed to the Stock bucket and rebuilt to My
   Portfolio Trades' six-column-block layout.
2. **"Other Stock Options"**, moved *ahead* of Other Stock Holdings --
   a display-label and `app.py`-ordering change only.
3. **"Modified CSPs"** (this version), moved to sit right after My
   Current CSPs instead -- alongside the stock-holding exclusion above
   and the column changes below. The Stock LTP/Momentum/1D/5D/20D
   columns still work exactly as before, just now resolving the
   underlying's own `symbol` from a **Position** leg
   (`row["stock_symbol"]`, despite the name -- kept for the least
   line-churn against the shared duplicate of My Portfolio Trades'
   rendering code) instead of a Holding leg, since there's never a
   Holding leg to read it from anymore.

**Cash Needed** and **Credit** (new columns, replacing the removed
Stock Holding-derived ones) -- see `portfolio_service.modified_csp_cash_needed`/
`net_option_credit` for the exact rules: Cash Needed is the
cash-secured-put amount for a **naked** short put only (zero once any
PE Buy leg exists at all, covered or not); Credit is the whole trade's
net premium (every short leg's premium added, every long leg's
subtracted).
...
option Position legs, via `portfolio_page.load_trade_margin`. Needs a
connected Dhan account (Data Provider = Dhan); shows "N/A" otherwise, or
if the call fails (expired token, an unresolvable leg)."""
...
st.set_page_config(page_title="Modified CSPs | Nifty 50 Screener", page_icon="\U0001f4cb", layout="wide")
...
st.title("\U0001f4cb Modified CSPs")
render_disclaimer()
...
st.caption(
    'Every stock Trade with at least one option leg and **no stock holding at all** whose Trade Type is '
    'neither "CSP" nor starts with "Portfolio " -- a bare Strangle/Jade Lizard/Twisted Sister/IC, or the '
    "default \"Trade\" label on a naked option position: the shapes a CSP typically turns into once it's been "
    "rolled or adjusted. A Trade with a stock holding never shows here, even under a custom label -- see My "
    "Portfolio Trades or Other Stock Holdings for those. Index and Other-bucket trades aren't shown here "
    'either -- see the unfiltered "All Trades" page under My Portfolio. "Cash Needed" is the cash-secured-put '
    "amount for a naked short put only (zero once any put buy leg exists at all). \"Credit\" is the whole "
    'trade\'s net premium -- credits from every sell leg minus debits from every buy leg. "Margin Required" is '
...
# blocks (it still counts toward the trade's P&L above).
...
def _render_modified_csps_table(*, trades: list[dict], portfolio_name: str, dhan_connection) -> None:
    if not trades:
        st.caption('No trades tagged neither "CSP" nor "Portfolio ..." (with no stock holding) yet for this portfolio.')
        return

    # Every trade here has zero Holding legs (excluded before this
    # function is even called -- see _render_modified_csps_tab), so the
    # underlying's own symbol is read off a Position leg instead of a
    # Holding leg, unlike My Portfolio Trades' identical-looking
    # aggregation this was originally copied from.
    per_trade = []
    has_unshown_legs = False
    for t in trades:
        position_legs = [leg for leg in t["legs"] if leg["leg_type"] == "Position"]
        stock_symbol = next((leg["symbol"] for leg in position_legs if leg["symbol"]), None)
    ...
    # exists (src.calculations.returns.live_return_1d), so Momentum
    # reacts to the same live price as Stock LTP instead of staying
    # pinned to the stored EOD-vs-EOD figure -- falls back to the stored
    # return_1d unchanged otherwise.
    returns_by_symbol = {
    ...
    for row in per_trade:
        ...
        cash_needed = portfolio_service.modified_csp_cash_needed(row["slots"]["PE Sell"], row["slots"]["PE Buy"])
        credit = portfolio_service.net_option_credit(row["position_legs"])
    ...
    if has_unshown_legs:
        st.caption(
            ...
            "different strikes) -- only the first is shown in that slot here (Cash Needed/Credit still account "
            "for every leg, not just the first). See My Trades/Analyse Trade for the full leg list."
        )
    ...
    st.dataframe(
        ...
        hide_index=True,
        key=f"modified_csps_table_{slug(portfolio_name)}",
...
def _render_modified_csps_tab(
    ...
    # Exclude any Trade with a stock holding, per an explicit user
    # request -- those belong on My Portfolio Trades (if renamed to start
    # with "Portfolio ") or Other Stock Holdings instead, regardless of
    # what trade_type string a mixed holding+options Trade happens to
    # carry (e.g. a hand-typed "Hedged" used to show up here with its
    # Stock Holding numbers populated; now it's excluded outright).
    ...
    _render_modified_csps_table(trades=stock_option_trades, portfolio_name=portfolio_name, dhan_connection=dhan_connection)
