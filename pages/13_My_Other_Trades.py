...
st.set_page_config(page_title="My Other Trades | Nifty 50 Screener", page_icon="\U0001f4e6", layout="wide")
...
st.title("\U0001f4e6 My Other Trades")
render_disclaimer()
...
st.caption(
    'Every Trade from My Trades whose Trade Type is neither "CSP" nor "Covered Call" -- including the default '
    '"Trade" label and any other strategy (Strangle, Jade Lizard, Twisted Sister, ...). Go to My Trades, select '
...
def _render_trades_table(*, title: str, trades: list[dict], portfolio_name: str, key_suffix: str) -> None:
    ...
    event = st.dataframe(
        ...
        key=f"other_table_{key_suffix}_{slug(portfolio_name)}",
        column_config={
    ...
    if selected_rows:
        ...
        if st.button(
            ...
            key=f"other_analyse_{key_suffix}_{slug(portfolio_name)}",
        ):
