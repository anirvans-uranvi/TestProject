...
# plan a covered call for a plain holding (Planner for CCs) -> see
# stocks that have been assigned into holdings with option overlays --
# with "Index Options" (strangle ideas on the 4 major indices) as its
# own section alongside it. (Modified CSPs went through two renames and
# a reordering, all per separate explicit requests: "Other Stock Trades"
...
# see that page's own docstring. Planner for CCs is a full rebuild of
# the earlier "Other Stock Holdings" -- same file, same underlying
# filter, renamed and moved from after My Portfolio Trades to right
# after Modified CSPs; see its own docstring too.) "Market" (raw
